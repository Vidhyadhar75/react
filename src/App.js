...
const App = () => {
  ...
  const handleButtonClick = async () => {
    ...
    try {
      const response = await fetch(`https://ho-2.onrender.com/${buttonState ? 'true' : 'false'}`, {
        method: 'POST',
        headers: {
  ...
  const handleFanClick = async () => {
    ...
    try {
      const response = await fetch(`https://ho-2.onrender.com/${fanState}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleLightClick = async () => {
    ...
    try {
      const response = await fetch(`https://ho-2.onrender.com/${lightState}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleMotorClick = async () => {
    ...
    try {
      const response = await fetch(`https://ho-2.onrender.com/${motorState}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  useEffect(() => {
    const socket = new WebSocket('https://ho-2.onrender.com');
